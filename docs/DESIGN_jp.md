...
6. **採用するASP.NET Core向けInertiaアダプタの選定** — `InertiaNetCore` /
   `InertiaCore` / `inertia-dotnet` 等、明確な単一標準のない複数のコミュニティフォークが
   存在し、まだ比較・決定していない (`HANDOFF.md` §6.3/§6.4)。`Inertia.Render` の
   呼び出し箇所の検出方法 (§3.5) や、Shared Data登録の発見可能性 (§8.2) に影響する。
   **本書はこの選定を意図的に確定していない** ので、特定の選択がなされたと仮定しない
   こと。
7. **想定するフロントエンドフレームワーク** — React / Vue / Svelte — 生成する
   Props型の形 (`interface Props { ... }` か、Vueの `defineProps<...>()` 志向の型か等)
   に影響する。まだ未決定。§2.6、§7.4、および§5.4のジェネリック引数の形にも影響する。
   **こちらも特定の選択がなされたと仮定しないこと。**
...
- **ASP.NET Core向けInertia.jsアダプタ (`InertiaNetCore`/`InertiaCore`/
  `inertia-dotnet`等) の成熟度や、フロントエンドフレームワークの選定については、
  まだWeb調査を行っていない。** これは意図的なものであり、本書がライブラリ・
  フレームワークの選択を先走って確定させてしまわないよう、調査・決定は行わず
  §11項目6–7の未解決事項として明記するに留めている (`HANDOFF.md` §6.4)。
