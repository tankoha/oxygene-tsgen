...
10. インクリメンタル生成・Watchモード・CI差分チェック (§9) — CLIの基本機能が
    安定してから開発体験向上として着手
11. **ランタイム値の出力モード: ambient な `.d.ts` の代わりに (あるいは併せて)
    実体のある `.ts` モジュールを出力する。** 2026-08-13、`TeaTimeTracker` が
    手書き型を生成型に差し替える作業中に提起。完全な差し替えを阻んだ唯一の
    要因がこれ。`.d.ts` の `declare namespace ... export enum` は ambient 宣言
    であり、enum に**型はあるが実行時の値が無い**。enum メンバを実値として
    使うフロントエンド (比較、計算プロパティキー — `ThemeMode.Light`、
    `BeverageType.GreenTea`) は生成物からそれを得られず、全 enum の手書きの
    複製を保持し続けるしかない。実体のある `export enum` (あるいは
    `export const X = {...} as const` + リテラルユニオン型) を持つモジュール
    を出力できればこのギャップは解消する。これは `DtsEmitter` のフラグでは
    済まない、出力形態そのものの変更である点に注意: モジュールの `export` は
    ambient なグローバルではないため、利用側は `TeaTimeTracker.Models.Foo`
    という素の参照から import に切り替わり、2つのモードを単純に1ファイルへ
    連結することもできない (§10.1 の「単一/分割ファイル切替」が最も近い
    先例)。interface 自体はどちらのモードでも影響を受けない — これは
    「実行時に存在する必要がある宣言」だけの話であり、現時点ではそれは
    enum を意味する。
