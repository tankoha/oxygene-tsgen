...
**解決 (2026-08-07): typoであることが確認され、実際の期限は30日間。**
RemObjectsサポートから2026-08-02のフォローアップへの返信があった:
「Trial edition is for no longer than "3 days." I was initially under
the impression that the trial period was actually 30 days. [...] That
was a typo, sorry, yes. it's 30 days.」これにより未解決事項は解消 —
Trial版の使用期限はインストール日(2026-08-01)から30日間であり、3日間
ではない。これに伴い課題管理表(`reports/2026-08-02-issue-tracker.csv`)
の項目#19はクローズした。`CLAUDE.md`のライセンス制約の節もこの内容を
反映するよう更新済み。
...
2. **§9.2のTrial期間に関する質問。** ユーザーが別途、Marcに直接
   ...
   見直す価値がある。
   **解決 (2026-08-07): typoであることが確認され、実際の期限は30日間。
   返信全文は§9.2の更新箇所を参照。** 課題管理表の項目#19はクローズ
   した。
...
## 25. 自己レビューによる修正: CLI引数解析に境界チェックがなかった(Windows実機検証、2026-08-02)

ユーザーが「今回のセッションのコードにAI生成コードにありがちな失敗
パターン(一貫性のムラ、過剰な防御的コーディング、依存関係の楽観性、
セキュリティ面の考慮漏れ、古い書き方)が出ていないか」と尋ねたことを
きっかけに行った自己レビューで発見。テストでは見つからなかった —
不正な形式のCLI呼び出しを検証するフィクスチャは存在せず、不正な形式/
エッジケースのOxygeneソースを検証するものしかないため。

**バグの内容**: `Program.pas`の引数解析ループは、値を取る全てのフラグ
(`--assembly`、`--source`、`--out`、`--enum-style`、
`--nrt-unknown-policy`、`--mode`)について`inc(i); x := args[i];`という
処理をしており、`(i < args.Length)`のガードが一切なかった。
`tsgen generate --assembly`を何も続けずに実行すると、綺麗なCLIエラー
メッセージではなく、未処理の`IndexOutOfRangeException`と生の.NET
スタックトレースでクラッシュしていた。

**単なるバグというより、実際の不一致として名指しする価値がある**:
同じセッションの`InertiaScanner.pas`(§24)は、トークンリストへの
アクセスのほぼ全箇所を`(i < aTokens.Count)`でガードしている —
トークン走査コードでは几帳面な境界チェックをしているのに、CLI自身の
引数解析には一切ない。同じコードベース、同じセッションで、扱っている
「ユーザー入力」の種類(Oxygeneソース vs コマンドライン引数)によって
慎重さのレベルが全く違う — ユーザーが尋ねていた「セッション内で
一貫性がぶれる」パターンの具体例である。

**修正**: `missingValueFor: String`という番兵を追跡し、各フラグの
分岐で、値が欠けている場合は範囲外アクセスの代わりにこれをセットする
ようにした。ループは`Error: --flag requires a value.`(終了コード1)
で抜けるようになり、そのまま処理を続けることはない。既存スイートを
再実行しただけでなく、実機で確認した: `tsgen generate --assembly`
(および別途`--nrt-unknown-policy`/`--mode`を値なしで実行した場合)は
綺麗なエラーを表示して終了コード1になり、フルのスナップショット
スイートは14/14で変わらずパスすることを確認 — 正常な、正しい形式の
呼び出しに退行がないことを確認した。エラーパス自体に対する新規の
自動フィクスチャは追加していない — `tools/run-tests.ps1`は「アセンブリ
をビルドし、それに対して`tsgen`を実行する」ことを前提にした作りで
あり、不正な形式の呼び出しをテストするものではない。この1件の修正の
ためにその基盤を追加する価値があるとは今回は判断しなかった(手動検証
は簡単であり、実際に行った — このプロジェクトの過去の一部の事例
[「動くはず」と確認せずに主張してしまったケース]とは違う)。

**同じレビュー中に別途気づいた、まだ直していない点**: §24.2の
`InertiaScanner.UnquoteStringLiteral`の正しさに関する主張が、実際に
検証した内容より一歩踏み込みすぎていたことが判明した —
`GetString()`/`GetOriginalString()`がクォートを外さないことは実機
確認済みだったが、`Replace('''''', ''')`によるアンエスケープ処理
自体は、どのフィクスチャからも一度もテストされていなかった
(`tests/fixtures/InertiaMode`の文字列リテラルにはエスケープされた
クォートが含まれていない)。この同じレビュー中に、使い捨ての
scratchpadプローブで`'it''s'`をトークナイズして検証した: 正しく
`it's`(4文字)にアンエスケープされることを確認した。コード変更は
不要だった — これは実際のバグではなく、ドキュメントの精度の問題
(実際に検証した以上のことを検証済みと主張してしまっていた)だった。
それ自体1つの教訓として覚えておく価値がある: このファイルにおける
「実機確認済み」という主張は、大まかな領域ではなく、正確に何を
テストしたかを名指しすべきである。

### 25.1. 同じ自己レビューの残り3観点

ユーザーの依頼には、AI生成コードにありがちな失敗パターンが5つ挙がって
いた。うち2つは既に上で扱った(一貫性のムラ → バグ修正、依存関係の
楽観性 → `UnquoteStringLiteral`の注記)。残る3つも同じレビュー中に
確認したが、コード変更には至らなかった。それでも結論を記録しておけば、
次のセッションで同じ検討をやり直さずに済む:

- **過剰な防御的コーディング**: 目立った例は見つからなかった。
  どちらかというと逆方向(上記のバグ)に振れていた — 各機能追加は
  依頼された範囲に収まるよう都度スコープを確認しながら進めたため、
  求められていないエッジケース処理を積み増す方向にはならなかった。
- **セキュリティ面の考慮漏れ**: このプロジェクトの実際の脅威モデル
  においては該当性が低い — `tsgen`はシークレットもIAMもSQLも扱わない
  ローカルCLIである。唯一の候補である`--assembly`/`--source`が
  未検証のファイルパスを受け取る点についても、ガードする価値なしと
  判断した: このツールは呼び出したユーザー自身のマシン上の自分の
  アセンブリ/ソースしか指さないため、パスを検証しないことで越えて
  しまう権限境界が存在しない。
- **古い書き方**: 文字通りの「非推奨API使用」ではない(このコード
  ベースにはそこまで古いものは何もない)が、近い現象が2回起きて
  いる — C#に馴染みのある構文がOxygeneでもそのまま通じるはずだと
  無意識に仮定してしまうことである。両方とも既に別の箇所で記録済み
  なので、繰り返さずここでは相互参照のみとする: 黙って空処理になる
  オブジェクト初期化子構文(§22.3)と、2回発生した`{ }`コメントの
  ネスト不可バグ(§23.6、§24.3。`CLAUDE.md`にも定番の落とし穴として
  記載済み)。どちらも根本原因は同じ — C#での訓練データへの馴染みが
  Oxygeneの構文についての思い込みに漏れ出したこと — なので、今後
